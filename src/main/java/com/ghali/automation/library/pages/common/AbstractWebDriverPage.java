package com.ghali.automation.library.pages.common;
...
public abstract class AbstractWebDriverPage {
	private final static Logger LOG = Logger.getLogger(AbstractWebDriverPage.class);
	...
	/**
	 * Performs seek operation on element.
	 *
	 * @param element
	 * @param seekPercent
	 */
	public void seek(WebElement element, int seekPercent) {

		// int yAxis = element.getLocation().getY();

		// int x_cord = (int) (element.getSize().width) ;
		int toX = (element.getSize().width);
		toX = toX + seekPercent * 10;
		Actions actions = new Actions((WebDriver) this.driverInterface.getDriver());
		actions.moveToElement(element).clickAndHold().moveByOffset(toX, 0).release().perform();
